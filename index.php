...
{
      $file = $word_search['file'];
      logger(" File : ". $word_search["file"]. "\n"." Clave : ".$word_search["clave"]. "\n"." Codigo : ".$word_search["codigo"]);
...
function search_url($texto_completo, $data)
{
      ...
      logger("Dato retorno: ". $url);
      logger("Solicitud Finalizada ");
      return array(
            'parametro' => $url
      );
}
...
function logger($mensaje_log)
{
      $logFile = fopen('/opt/lampp/htdocs/app_wsdl_tablas/log.txt', 'a+');
      $text_log = "\n" . date(" d/m/Y H:i:s ")."\n". $mensaje_log ;
      fwrite($logFile, $text_log);
      fclose($logFile);
}
