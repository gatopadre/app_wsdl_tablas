...
require_once('nusoap-0.9.5/lib/nusoap.php');
//$log="log.txt";
$logFile = fopen('/opt/lampp/htdocs/app_wsdl_tablas/log.txt', 'a+') ;
fwrite($logFile, "\n".date("d/m/Y H:i:s")." ###Iniciando Solicitud###") or die("Error escribiendo en el archivo");fclose($logFile);
# constantes
define('HOST', 'http://servicio-tablas.cl/');
define('PYTHON_HOST', 'http://localhost:8085/');
...
// Parametros de entrada
$logFile = fopen('/opt/lampp/htdocs/app_wsdl_tablas/log.txt', 'a+') ;
fwrite($logFile, "\n".date("d/m/Y H:i:s")." Recibiendo parametros de entrada") or die("Error escribiendo en el archivo");fclose($logFile);
$server->wsdl->addComplexType(
      'in_data',
      'complexType',
      'struct',
...
// Parametros de Salida
$logFile = fopen('/opt/lampp/htdocs/app_wsdl_tablas/log.txt', 'a+') ;
fwrite($logFile, "\n".date("d/m/Y H:i:s")." Respondiendo Solicitud") or die("Error escribiendo en el archivo");fclose($logFile);
$logFile = fopen('/opt/lampp/htdocs/app_wsdl_tablas/log.txt', 'a+') ;
fwrite($logFile, "\n".date("d/m/Y H:i:s")." ###Finalizando Solicitud###") or die("Error escribiendo en el archivo");fclose($logFile);
$server->wsdl->addComplexType(
      'out_data',
      'complexType',
      'struct',
...

{
      ...
      fwrite($logFile, "\n".date("d/m/Y H:i:s")." ###Iniciando Solicitud###") or die("Error escribiendo en el archivo");fclose($logFile);
            $logFile = fopen('/opt/lampp/htdocs/app_wsdl_tablas/log.txt', 'a+') ;
      fwrite($logFile, "\n".date("d/m/Y H:i:s").$query) or die("Error escribiendo en el archivo");fclose($logFile);
            ...
            $logFile = fopen('/opt/lampp/htdocs/app_wsdl_tablas/log.txt', 'a+') ;
      fwrite($logFile, "\n".date("d/m/Y H:i:s").$url) or die("Error escribiendo en el archivo");fclose($logFile);
